...
int main() {
...
#ifdef __USE__GLFW__
	GLFWwindow* window = initWindow("RTIOW: Ch.06 - Normals",WIDTH, HEIGHT);
#endif
#ifdef __USE__SDL2___
	SDLWindow* window = initWindow(WIDTH, HEIGHT);
#endif
	...
	getRequiredInstanceExtensions(window, requiredExtensions);
...
#define COMPUTE_HEIGHT 608
	...
	VkShaderModule compShader = initShaderModule(device, "Shaders/raytrace06.comp.spv");
	...
	for (size_t i = 0; i < swapchainImages.size(); i++) {
		renderFences[i] = initFence(device, VK_FENCE_CREATE_SIGNALED_BIT);
	}
	...
	while (!glfwWindowShouldClose(window)) {
		...
		vkCmdDispatch(computeCommandBuffer, 1024 / 16, 1024 / 16, 1);
		vkEndCommandBuffer(computeCommandBuffer);
		computeInfo.pCommandBuffers = &computeCommandBuffer;
		res = vkQueueSubmit(computeQueue, 1, &computeInfo, computeFence);
		assert(res == VK_SUCCESS);
		res = vkAcquireNextImageKHR(device, swapchain, UINT64_MAX, presentComplete, nullptr, &index);
